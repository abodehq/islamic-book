...
public class PublishersActivity extends Activity {

	private PublishersActivity _scope;
	private GetTask getTask;

	private ArrayList<HashMap<String, String>> booksList;
	private ListView list;
	private PublisherItemAdapter authorItemAdapter;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		iconContextMenu.addItem(res, getString(R.string.context_menu_publisher_books), R.drawable.ic_1,
				MENU_ITEM_1_ACTION);
		iconContextMenu.addItem(res, getString(R.string.context_menu_publisher_info),
	...
	private class GetTask extends AsyncTask<Void, Void, ReturnModel> {
		...
		protected void onPostExecute(ReturnModel result) {
			...
			authorItemAdapter = new PublisherItemAdapter(_scope, booksList);
	...
	private void ShowPdfViewer(int bookPosition) {
		PublishersBooksActivity.publisherId = booksList.get(bookPosition).get("publisherId");
		Intent i = new Intent(getApplicationContext(), PublishersBooksActivity.class);
	...
	private void showAddToPlaylist() {
		String authorInfo = booksList.get(selectedBookIndex).get("publisherInfo").toString();
		if(authorInfo.length()==0)
			authorInfo = getString(R.string.publisher_no_info);
		ErrorDialog errorDialog= new ErrorDialog(PublishersActivity.this,authorInfo);
		errorDialog.show();
	}
		...
		try {
			if (lastselectedImagePosition == -1)
				lastselectedImagePosition = 0;
					Object result = amfConnection.call("publishersService.getAllPublishersWithDetails",
			...
			try {
				...
				for (int i = 1; i < jsonArray.length(); i++) {
					...
					book.put("publisherName", jsonObject.getString("publisherName"));
					book.put("publisherInfo", jsonObject.getString("publisherInfo"));
	...
	protected Dialog onCreateDialog(int id) {
		if (id == CONTEXT_MENU_ID) {
			return iconContextMenu.createMenu(getString(R.string.context_menu_publisher_title));
		}
